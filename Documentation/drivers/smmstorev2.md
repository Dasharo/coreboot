...
`%ebx` contains a number of detected capsules. If it equals 0, commands
for accessing full flash are permanently disabled, otherwise they are
permanently enabled until next boot. It is expected that the payload
won't allow regular OS to boot if the handler is enabled without
rebooting first.
