...
#
# Without --windows the cabinet contains firmware.bin + firmware.metainfo.xml.
# With    --windows it also contains firmware.inf and, when a signing certificate
# is provided, firmware.cat — all in the same .cab file.
...
function usage() {
    echo "Usage: $0 [--windows [--cert cert.pem --key key.pem | --gen-cert]] <capsule>" 1>&2
    echo "" 1>&2
    echo "  --windows          Include Windows Update files (firmware.inf, firmware.cat)" 1>&2
    echo "                     in the cabinet alongside the LVFS files" 1>&2
    ...
    echo "" 1>&2
    echo "  Requires: gcab, osslsigncode (for --windows --cert/--key/--gen-cert)" 1>&2
    exit 1
}
...
if $gen_cert && ! $windows_mode; then
    die "--gen-cert requires --windows"
fi
...
if $windows_mode; then
    ...
    # Optionally generate a self-signed code-signing certificate
    if $gen_cert; then
        cert_base=$(echo "${CONFIG_MAINBOARD_SMBIOS_PRODUCT_NAME}" | tr '[:upper:]' '[:lower:]')
        ...
        openssl req -x509 -newkey rsa:2048 \
            ...
            -days 3650 \
            -nodes \
            -subj "/CN=${CONFIG_MAINBOARD_SMBIOS_PRODUCT_NAME} Firmware Signing/O=${mfg_name}" \
        ...
        echo "WARNING: Self-signed certificate — for test signing only."
        echo "         Enable on Windows: bcdedit /set testsigning on"
    fi
...
if $windows_mode; then
    ...
    gcab -c "$cab" firmware.bin firmware.metainfo.xml
fi
popd &> /dev/null
