...
static int usage (char *name)
{
	...
	printf ("tmp_buf    - physical address of a buffer in low 4GB of RAM. Up to 8KB\n"
		...
		"             the CBMEM console).\n"
		"com_buf    - physical address of SMM COMBUFFER, can be read from\n"
		"             `cbmem -l`. It must be aligned to a page size.\n"
		"command    - command to execute, see `coreboot/src/include/smmstore.h`.\n"
		"             For SMMSTOREv2 this is 5 for read, 6 for write, 7 for\n"
		"             clear.\n"
		"param1 ... - params passed to SMI handler, as set of uint32_t values.\n"
		"             Expected structure depends on the command, refer to\n"
		"             `smmstore_params_raw*` structure definitions in\n"
		"             `coreboot/src/include/smmstore.h` file. At least one\n"
		"             parameter must be specified.\n"
		"in_file    - optional, binary file which content is written to com_buf\n"
		"             before SMM call.\n"
		"out_file   - optional, path to binary file (it is created if doesn't exist)\n"
