 ...
 *
 * Need to run this from master processor to all slave processors for
 * Secureboot hole (need to ensure that SP didn't leave compromised P8 Slave).
 *
 * - Update SBE config data area with any configs/parameters required by SBE
 *   (see step 0 for more details)
...
static void setup_sbe_config(uint8_t chip)
{
	...
	/*
	 * TODO: if OBUS support is needed:
	 *
	 * - set OPTICS_CONFIG_MODE_OBUS[0-3] bits
	 * - set OB[0-3]_PLL_BUCKET bits
	...
	boot_flags = (read_scom(0, MBOX_SCRATCH_REG1 + 2) >> 32);
...
static int get_master_sbe_boot_seeprom(void)
{
	...
	return (read_scom(0, PERV_SB_CS_SCOM) & SBE_BOOT_SELECT_MASK) ? 1 : 0;
}
