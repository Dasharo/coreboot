...
#include <vendorcode/ibm/power9/pstates/p9_pstates_pgpe.h>
			...
			const struct voltage_bucket_data *poundV_bucket)
{
	...
	const uint16_t socket_power_w = poundV_bucket->sort_power_turbo;
	const uint16_t sort_power_freq_mhz = poundV_bucket->turbo.freq;
	...
	for (i = 0; i < entry_count; ++i) {
		...
		if (rdev_readat(wof_device, tbl_hdr_buf, entries[i].offset,
			...
			const OCCPstateParmBlock *oppb)
{
	...
	if (src->magic_number != VFRT_HDR_MAGIC)
			...
			const OCCPstateParmBlock *oppb)
{
	...
	table_data = rdev_mmap(wof_device, entry.offset, entry.size);
	if (!table_data)
	...
	for (i = 0; i < tbl_hdr->vdn_size * tbl_hdr->vdd_size * MAX_QUADS_PER_CHIP; ++i) {
		import_vfrt((const struct vfrt_hdr *)wof_vfrt_entry, homer_vfrt_entry, oppb);
			...
			const struct voltage_bucket_data *poundV_bucket)
{
	...
	if (hdr->magic_number != WOF_IMAGE_MAGIC_VALUE)
	...
	entries = rdev_mmap(wof_device, hdr->offset, hdr->entry_count * sizeof(entries));
	if (!entries)
...
void build_parameter_blocks(uint8_t chip, struct homer_st *homer, uint64_t functional_cores)
{
	...
	cme_hdr->custom_length =
		ALIGN_UP(cme_hdr->max_spec_ring_len * 32 + sizeof(LocalPstateParmBlock), 32) /
		32;
