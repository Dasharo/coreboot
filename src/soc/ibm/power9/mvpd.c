...
static int read_eeprom(uint32_t offset, void *data, uint32_t len)
{
	...
	if (read_eeprom_chip(offset, data, len1))
		return 1;
	if (read_eeprom_chip(EEPROM_CHIP_SIZE, (uint8_t *)data + len1, len2))
		return 1;
			       ...
			       uint8_t *buf, size_t *size)
{
	...
	if (read_eeprom(offset, &record_size, sizeof(record_size)))
	...
	if (read_eeprom(offset, name, sizeof(name)))
	...
	while (offset < record_size) {
		...
		if (read_eeprom(offset, name_buf, sizeof(name_buf)))
		...
		if (name_buf[0] == '#') {
			...
			if (read_eeprom(offset, &kwd_size, sizeof(kwd_size)))
				die("Failed to read large keyword size from EEPROM\n");
			kwd_size = le16toh(kwd_size);
			offset += 2;
		} else {
			uint8_t small_size;
			if (read_eeprom(offset, &small_size, sizeof(small_size)))
		...
		if (!memcmp(name_buf, kwd_name, VPD_KWD_NAME_LEN) && index-- == 0) {
			...
			if (read_eeprom(offset, buf, kwd_size))
...
static const uint8_t *mvpd_get(void)
{
	...
	static const char *const mvpd_records[] = {
		"CRP0", "CP00", "VINI",
	...
	for (i = 0; i < 3; ++i) {
		...
		for (j = 0; j < entry_count; ++j) {
			...
			if (mvpd_offset + record_size > SECTION_SIZE) {
				die("MVPD section doesn't have space for %.4s record of "
			...
			if (read_eeprom(record_offset, mvpd_buf + mvpd_offset, record_size))
