...
static void mem_pll_setup(uint8_t chip)
{
	...
	for (i = 0; i < MCS_PER_PROC; i++) {
		...
		*/
		write_scom_for_chiplet(chip, mcs_ids[i], PCBSLMC01_NET_CTRL1_WAND,
		...
		*/
		...
		assert([0] (reserved) == 1)
		*/
		if (!(read_scom_for_chiplet(chip, mcs_ids[i],
		...
		*/
		scom_and_or_for_chiplet(chip, mcs_ids[i], MCSLOW_OPCG_ALIGN,
		                        ~PPC_BITMASK(47,51),
		...
		*/
		write_scom_for_chiplet(chip, mcs_ids[i], PCBSLMC01_ERROR_REG, ~0);
		...
		*/
		scom_and_for_chiplet(chip, mcs_ids[i], PCBSLMC01_SLAVE_CONFIG_REG,
		                     ~PPC_BIT(12));
	}
}
