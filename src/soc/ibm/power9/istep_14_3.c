...
static void init_pecs(uint8_t chip, const uint8_t *iovalid_enable)
{
	...
	for (pec = 0; pec < MAX_PEC_PER_PROC; ++pec) {
		 ...
		 *
		 * if HW423589_option1, set Disable Group Scope (r/w) and Use Vg(sys) at Vg
		 * scope
		 */
		...
		if (!node_pump_mode) {
			 ...
			 *
			 * DISABLE_CI      = 0x0 - Disable cache inject completely.
			 *                         (Reset value default.)
			 * P7_STYLE_CI     = 0x1 - Use cache inject design from Power7.
			 * PCITLP_STYLE_CI = 0x2 - Use PCI TLP Hint bits in packet to perform
			 *                         the cache inject.
			 * P9_STYLE_CI     = 0x3 - Initial attempt as cache inject. Power9
			 *                         style. (Attribute default.)
			 *
			 * Different cache inject modes will affect DMA write performance. The
			 * attribute default was selected based on various workloads and was to
			 * be the most optimal settings for Power9.
			 *
			...
			if (dd == 0x21 || dd == 0x22 || dd == 0x23) {
				 ...
				 * Attribute to control the cache inject throttling when cache
				 * inject is enable.
				 *
				 * DISABLE   = 0x0 - Disable cache inject throttling.
				 *                   (Reset value default.)
				 * 16_CYCLES = 0x1 - Perform 1 cache inject every 16 clock
				 *                   cycles.
				 * 32_CYCLES = 0x3 - Perform 1 cache inject every 32 clock
				 *                   cycles. (Attribute default.)
				 * 64_CYCLES = 0x7 - Perform 1 cache inject every 32 clock
				 *                   cycles.
				 *
				 * Different throttle rates will affect DMA write performance.
				 * The attribute default settings were optimal settings found
