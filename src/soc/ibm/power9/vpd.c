...
static char mapping_lookup(const struct vpd_info *vpd, const uint8_t *mapping, size_t size)
{
	int i = 0;
	int entry_count = 0;
	int offset = 0;
...
static const uint8_t *find_vpd_conf(const struct vpd_info *vpd, const char *mapping_name,
				    size_t *size)
{
...
static void load_mt_attrs(const uint8_t *mt_conf, size_t size, int vpd_idx)
{
	...
	case 0:
		...
		if (vpd_idx == 0) {
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_ACBOOST_RD_UP, &mt_conf[86], 4);
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_ACBOOST_WR_DOWN, &mt_conf[94], 4);
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_ACBOOST_WR_UP, &mt_conf[102], 4);

			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_CTLE_CAP, &mt_conf[110], 8);
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_CTLE_RES, &mt_conf[126], 8);
			...
			memcpy(&ATTR_MSS_VPD_MT_WINDAGE_RD_CTR, &mt_conf[214], 2);
		}
		break;
	case 1:
		memcpy(&ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][0][0], &mt_conf[172], 2);
		memcpy(&ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][1][0], &mt_conf[176], 2);
		memcpy(&ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][0][0], &mt_conf[188], 2);
		memcpy(&ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][1][0], &mt_conf[192], 2);
		ATTR_MSS_VPD_MT_VREF_DRAM_WR[vpd_idx] = mt_conf[206];
		memcpy(&ATTR_MSS_VPD_MT_VREF_MC_RD[vpd_idx], &mt_conf[208], 4);

		/* The following data is the same for all configurations */
		if (vpd_idx == 0) {
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_ACBOOST_RD_UP, &mt_conf[88], 4);
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_ACBOOST_WR_DOWN, &mt_conf[96], 4);
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_ACBOOST_WR_UP, &mt_conf[104], 4);

			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_CTLE_CAP, &mt_conf[112], 8);
			memcpy(&ATTR_MSS_VPD_MT_MC_DQ_CTLE_RES, &mt_conf[128], 8);
			...
			memcpy(&ATTR_MSS_VPD_MT_WINDAGE_RD_CTR, &mt_conf[216], 2);
		}
		break;
	}
...
static void load_mr_attrs(const uint8_t *mr_conf, size_t size, int vpd_idx)
{
	...
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CMD_ADDR_CASN_A15[vpd_idx], &mr_conf[66], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CMD_ADDR_RASN_A16[vpd_idx], &mr_conf[68], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CMD_ADDR_WEN_A14[vpd_idx], &mr_conf[70], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CMD_PAR[vpd_idx], &mr_conf[72], 2);

	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D0_CKE0[vpd_idx], &mr_conf[74], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D0_CKE1[vpd_idx], &mr_conf[76], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D0_CSN0[vpd_idx], &mr_conf[82], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D0_CSN1[vpd_idx], &mr_conf[84], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D0_ODT0[vpd_idx], &mr_conf[90], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D0_ODT1[vpd_idx], &mr_conf[92], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D1_CKE0[vpd_idx], &mr_conf[78], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D1_CKE1[vpd_idx], &mr_conf[80], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D1_CSN0[vpd_idx], &mr_conf[86], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D1_CSN1[vpd_idx], &mr_conf[88], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D1_ODT0[vpd_idx], &mr_conf[94], 2);
	memcpy(ATTR_MSS_VPD_MR_MC_PHASE_ROT_CNTL_D1_ODT1[vpd_idx], &mr_conf[96], 2);
}
