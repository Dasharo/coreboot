...
{
	const struct device* dev = pcidev_path_on_bus(routing_info->devfn >> 8,
						      routing_info->devfn & 0xff);
...
const struct pci_routing_info *get_pci_routing_table(size_t *entries)
{
	...
	for (size_t d = 0; d < MAX_DOMAINS; d++) {
		...
		routing_table[idx].devfn = PCI_DEVFN(0, 3);
		routing_table[idx].devfn |= ((d * BUSES_PER_DOMAIN) << 8);
		printk(BIOS_DEBUG, "%02x:%02x.%x: group: %u, swizzle: %u, irq: %u\n",
			routing_table[idx].devfn >> 8,
			PCI_SLOT(routing_table[idx].devfn),
			PCI_FUNC(routing_table[idx].devfn),
		...
		for (size_t i = 0; i < max_rbs; i++) {
			...
			routing_table[idx + i].devfn = dev_remap.fn_num;
			routing_table[idx + i].devfn |= (dev_remap.dev_num << 3);

			/* Workaround empty remap for bridges 7.1 and 7.2 */
			if (i >= 20 && routing_table[idx + i].devfn == 0) {
				routing_table[idx + i].devfn = PCI_DEVFN(7, i - 19);
			...
			routing_table[idx + i].devfn |= ((d * BUSES_PER_DOMAIN) << 8);
			...
			printk(BIOS_DEBUG, "%02x:%02x.%x: group: %u, swizzle: %u, irq: %u\n",
			       routing_table[idx + i].devfn >> 8,
			       PCI_SLOT(routing_table[idx + i].devfn),
			       PCI_FUNC(routing_table[idx + i].devfn),
