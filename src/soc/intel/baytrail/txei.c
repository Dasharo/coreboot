...
static uintptr_t get_txei_bar1(pci_devfn_t dev)
{
	uintptr_t bar;

	bar = pci_read_config32(dev, PCI_BASE_ADDRESS_1);
	bar &= ~PCI_BASE_ADDRESS_MEM_ATTR_MASK;
	assert(bar != 0);
	return bar;
}
...
int txe_aliveness_request(uintptr_t tempbar, bool request)
{
	...
	if (request)
		write32p(tempbar + TXE_MMIO_ALIVENESS_REQ,
			 read32p(tempbar + TXE_MMIO_ALIVENESS_REQ) | TXE_ALIVENESS_REQ);
	else
		write32p(tempbar + TXE_MMIO_ALIVENESS_REQ,
			 read32p(tempbar + TXE_MMIO_ALIVENESS_REQ) & (uint32_t)~TXE_ALIVENESS_REQ);
...
void txe_txei_init(void)
{
	...
	/* Ensure the BARs are the same as we just set them */
	tempbar0 =  pci_read_config32(dev, PCI_BASE_ADDRESS_0);
	tempbar0 &= ~PCI_BASE_ADDRESS_MEM_ATTR_MASK;
	tempbar =  pci_read_config32(dev, PCI_BASE_ADDRESS_1);
	tempbar &= ~PCI_BASE_ADDRESS_MEM_ATTR_MASK;
	...
	stopwatch_init_usecs_expire(&sw, HECI_READY_TIMEOUT);
	do {
		aliveness_req = read32p(tempbar + TXE_MMIO_ALIVENESS_REQ) & TXE_ALIVENESS_REQ;
		aliveness_resp = read32p(tempbar + TXE_MMIO_ALIVENESS_RESP) & TXE_ALIVENESS_ACK;
		if (stopwatch_expired(&sw)) {
	...
	if ((read32p(tempbar + TXE_MMIO_ALIVENESS_REQ) & TXE_ALIVENESS_REQ) == TXE_ALIVENESS_REQ)
	...
	write32p(tempbar + TXE_MMIO_S_SEC_IPC_OUTPUT_STS,
		 read32p(tempbar + TXE_MMIO_S_SEC_IPC_OUTPUT_STS) | IPC_OUTPUT_READY);
...
static uint32_t read_bar1(pci_devfn_t dev, uint32_t offset)
{
	return read32p(get_txei_bar1(dev) + offset);
}
...
static void write_bar1(pci_devfn_t dev, uint32_t offset, uint32_t val)
{
	return write32p(get_txei_bar1(dev) + offset, val);
}

static bool txe_ipc_ready(void)
{
...
{
	return ((read_bar1(PCI_DEV(0, TXE_DEV, TXE_FUNC), TXE_MMIO_SEC_IPC_READINESS) & IPC_READINESS_SEC_RDY) == 0);
}

static uint32_t read_slot(uint32_t idx)
{
	return read_bar1(PCI_DEV(0, TXE_DEV, TXE_FUNC), TXE_MMIO_IPC_OUTPUT_PAYLOAD + idx * SLOT_SIZE);
}

static void write_slot(uint32_t idx, uint32_t val)
{
	write_bar(PCI_DEV(0, TXE_DEV, TXE_FUNC), TXE_MMIO_IPC_INPUT_PAYLOAD + idx * SLOT_SIZE, val);
}
...
static int ipc_output_ready(void)
{
	...
	ipc_output_sts = read_bar1(PCI_DEV(0, TXE_DEV, TXE_FUNC), TXE_MMIO_S_SEC_IPC_OUTPUT_STS);
...
static void set_ipc_output_ready(void)
{
	uint32_t ipc_output_sts;
	ipc_output_sts = read_bar1(PCI_DEV(0, TXE_DEV, TXE_FUNC), TXE_MMIO_S_SEC_IPC_OUTPUT_STS);
	write_bar1(PCI_DEV(0, TXE_DEV, TXE_FUNC), TXE_MMIO_SEC_IPC_OUTPUT_STS, ipc_output_sts | 1);
}

static int wait_read_slots(void)
{
	struct stopwatch sw;
...
int txe_get_fw_version(struct get_fw_version_resp *reply)
{
	...
	printk(BIOS_DEBUG, "HECI: Get FW version Command\n");
	...
	printk(BIOS_DEBUG, "HECI: Get FW version %s!\n", !status ? "success" : "failure");
	return status;
}
