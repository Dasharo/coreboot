...
static void print_type20_info(const SIL_TYPE20_DMI_INFO *dmi20)
{
	printk(BIOS_SPEW,
		...
		dmi20->StartingAddr,
		dmi20->EndingAddr,
		dmi20->MemoryDeviceHandle,
		dmi20->MemoryArrayMappedAddressHandle,
		dmi20->PartitionRowPosition,
		dmi20->InterleavePosition,
		dmi20->InterleavedDataDepth,
		dmi20->ExtStartingAddr,
		dmi20->ExtEndingAddr);
}
...
void opensil_smbios_fill_cbmem_meminfo(void)
{
	...
	MemType = CONFIG(DRAM_SUPPORT_DDR4) ? 0 : SilDdr5Technology;
	status = xPrfGetSmbiosMemInfo(&SilContext, dmi_info, MemType);
	if (status != SilPass) {
		free(dmi_info);
		printk(BIOS_ERR,
	...
	for (unsigned int socket = 0;
		...
		for (unsigned int ch = 0;
			...
			for (unsigned int dimm = 0; dimm < SIL_MAX_DIMMS_PER_CHANNEL; dimm++) {
				...
				print_type20_info(&dmi_info->T20[socket][ch][dimm]);
