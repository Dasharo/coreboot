...
static u8 get_nb_rev(struct device *nb_dev)
{
	u8 reg;
	reg = pci_read_config8(nb_dev, 0x8);
	switch (reg & 3)
	{
	case 0x00:
		reg = REV_SR5650_A11;
 ...
 */
static void sr5650_set_tom(struct device *nb_dev)
{
	msr_t sysmem;
...
static void detect_and_enable_iommu(struct device *iommu_dev)
{
	...
	if (iommu) {
		...
		if (get_nb_rev(nb_dev) == REV_SR5650_A11) {
			printk(BIOS_DEBUG, "Disabling IOMMU IOTLB support\n");
			dword = pci_read_config32(iommu_dev, 0x6c);
			dword &= ~(0x1 << 8);	/* Disable IOTLB support */
...
static void sr5650_iommu_read_resources(struct device *dev)
{
	unsigned char iommu;
	struct resource *res;
...
static void sr5650_iommu_set_resources(struct device *dev)
{
	unsigned char iommu;
	struct resource *res;
...
static void sr5650_iommu_enable_resources(struct device *dev)
{
	detect_and_enable_iommu(dev);
}

static void sr5650_nb_pci_table(struct device *nb_dev)
