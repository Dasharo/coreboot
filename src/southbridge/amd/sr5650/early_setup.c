	...
	if ((cpu_ht_freq == 0x6) || (cpu_ht_freq == 0x5) || (cpu_ht_freq == 0x4) ||
		...
		if (CONFIG(NORTHBRIDGE_AMD_AMDFAM10)) {
			...
			*/
			 ...
			 */
			for (index = 0; index <= 0x1c; index += 4) {
				if (pci_read_config32(cpu_f0, 0x170 + index) & (1 << 8)) {
					ls2en_set = 1;
					break;
				}
			}
 ...
 */
static void sr5650_htinit_dect_and_enable_isochronous_link(void)
{
	pci_devfn_t sr5650_f0 = PCI_DEV(0, 0, 0);

	if (get_uint_option("iommu", 1)) {
...
static void sr5650_por_misc_index_init(pci_devfn_t nb_dev)
{
	...
	set_nbmisc_enable_bits(nb_dev, 0x00, 0x0000FFFF, 0 << 0 | 1 << 1 | 1 << 2 | 1 << 3 | 0 << 6 | 0 << 8);
