...
static void w83795_set_hystheresis_temp(struct device *dev)
{
	struct drivers_i2c_w83795_config *config = config_of(dev);

	w83795_write(dev, W83795_REG_HT(0), ((config->ht1_critical_hysteresis & 0x0f) << 4) |
					     (config->ht1_operation_hysteresis & 0x0f));
	w83795_write(dev, W83795_REG_HT(1), ((config->ht2_critical_hysteresis & 0x0f) << 4) |
					     (config->ht2_operation_hysteresis & 0x0f));
	w83795_write(dev, W83795_REG_HT(2), ((config->ht3_critical_hysteresis & 0x0f) << 4) |
					     (config->ht3_operation_hysteresis & 0x0f));
	w83795_write(dev, W83795_REG_HT(3), ((config->ht4_critical_hysteresis & 0x0f) << 4) |
					     (config->ht4_operation_hysteresis & 0x0f));
	w83795_write(dev, W83795_REG_HT(4), ((config->ht5_critical_hysteresis & 0x0f) << 4) |
					     (config->ht5_operation_hysteresis & 0x0f));
	w83795_write(dev, W83795_REG_HT(5), ((config->ht6_critical_hysteresis & 0x0f) << 4) |
					     (config->ht6_operation_hysteresis & 0x0f));
}

static void w83795_set_control_mode_table(struct device *dev)
{
	struct drivers_i2c_w83795_config *config = config_of(dev);
	struct w83795_control_mode_table *table = config->control_mode_table;
	u8 domain, point;

	for (domain = 0; domain < W83795_RELATIVE_TEMPERATURE_COUNT; domain++) {
		for (point = 0; point < W83795_SMART_FAN_CONTROL_POINTS; point++) {
			w83795_write(dev, W83795_REG_CTL_TABLE_TEMP(domain) + point,
				     table[domain].temperatures[point]);
			w83795_write(dev, W83795_REG_CTL_TABLE_PWM(domain) + point,
				     table[domain].temperatures[point]);
		}
	}
}
...
static void w83795_set_fan(struct device *dev, w83795_fan_mode_t mode)
{
	...
	if (mode == SMART_FAN_MODE) {
		...
		w83795_set_control_mode_table(dev);
	}
...
static void w83795_init(struct device *dev, w83795_fan_mode_t mode, u8 dts_src)
{
	...
	/* Thermal diode and resistor offsets */
	w83795_write(dev, W83795_REG_TEMP_OFFSET(0), temp_to_offset(config->td1_offset));
	w83795_write(dev, W83795_REG_TEMP_OFFSET(1), temp_to_offset(config->td2_offset));
	w83795_write(dev, W83795_REG_TEMP_OFFSET(2), temp_to_offset(config->td3_offset));
	w83795_write(dev, W83795_REG_TEMP_OFFSET(3), temp_to_offset(config->td4_offset));
