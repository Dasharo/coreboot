...
u8 is_ecc_enabled(struct MCTStatStruc *pMCTstat, struct DCTStatStruc *pDCTstat)
{
	...
	if (pDCTstat->NodePresent && (pDCTstat->DIMMValidDCT[0] || pDCTstat->DIMMValidDCT[1]))
		if (!(pDCTstat->status & (1 << SB_ECC_DIMMS)))
			ecc_enabled = 0;
				...
				struct DCTStatStruc *pDCTstat, u8 num_nodes)
{
	...
	interleaved = 0;
	if (pMCTstat->GStatus & (1 << GSB_Node_INTLV))
		interleaved = 1;
			...
			struct DCTStatStruc *pDCTstatA)
{
	 ...
	 *
	 * pDCTstat.cas_latency pDCTstat.speed
	 * j CL(j)       k   F(k)
	 * --------------------------
	 * 0 2.0         -   -
	 * 1 3.0         1   200 MHz
			...
			struct DCTStatStruc *pDCTstat, u8 dct, u8 enable)
{
	...
	if (enable) {
		...
		if (pDCTstat->dimm_x4_present & ((dct) ? 0xaa : 0x55))
			x4_present = 1;
		if (pDCTstat->dimm_x8_present & ((dct) ? 0xaa : 0x55))
			x8_present = 1;
...
retry_dqs_training_and_levelization:
	...
	if (nv_DQSTrainCTL) {
		mct_WriteLevelization_HW(pMCTstat, pDCTstatA, FIRST_PASS);
		...
		mct_WriteLevelization_HW(pMCTstat, pDCTstatA, SECOND_PASS);
		...
		for (Node = 0; Node < MAX_NODES_SUPPORTED; Node++) {
			...
			if (pDCTstat->NodePresent) {
				if (pDCTstat->train_errors & (1 << SB_FatalError)) {
					printk(BIOS_ERR, "DIMM training FAILED! Restarting system...");
					soft_reset();
				}
				if (pDCTstat->train_errors & (1 << SB_RetryConfigTrain)) {
					retry_requested = 1;

					/* Clear previous errors */
					pDCTstat->train_errors &= ~(1 << SB_RetryConfigTrain);
					pDCTstat->train_errors &= ~(1 << SB_NO_DQS_POS);
					pDCTstat->err_status &= ~(1 << SB_RetryConfigTrain);
					pDCTstat->err_status &= ~(1 << SB_NO_DQS_POS);
				}
			}
		}
		...
		if (retry_requested) {
			...
			for (Node = 0; Node < MAX_NODES_SUPPORTED; Node++) {
				...
				if (pDCTstat->NodePresent) {
					...
					u8 original_auto_speed = pDCTstat->dimm_auto_speed;
					...
					pDCTstat->dimm_auto_speed = original_auto_speed;
				}
			}
			...
			struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	*/
	...
	if (is_fam15h()) {
		...
		dword |= (pDCTstat->cas_latency & 0x1f);				/* Tcl */
		...
		val = pDCTstat->cas_latency - 4; /* pDCTstat.cas_latency to reg. definition */
		DramTimingLo |= val;
				...
				struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	tCKproposed16x =  16000 / pDCTstat->preset_max_freq;
	...
	for (;;) {
		...
		if (tCKproposed16x != min_frequency_tck16x) {
			if (pMCTstat->GStatus & (1 << GSB_EnDIMMSpareNW)) {
				pDCTstat->dimm_auto_speed = byte;
				pDCTstat->dimm_casl = bytex;
				break;
			} else {
	...
	printk(BIOS_DEBUG, "SPDGetTCL_D: dimm_casl %x\n", pDCTstat->dimm_casl);
	printk(BIOS_DEBUG, "SPDGetTCL_D: dimm_auto_speed %x\n", pDCTstat->dimm_auto_speed);

	printk(BIOS_DEBUG, "SPDGetTCL_D: status %x\n", pDCTstat->status);
	printk(BIOS_DEBUG, "SPDGetTCL_D: err_status %x\n", pDCTstat->err_status);
	printk(BIOS_DEBUG, "SPDGetTCL_D: err_code %x\n", pDCTstat->err_code);
				...
				struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (pDCTstat->logical_cpuid & (AMD_DR_Dx | AMD_DR_Cx | AMD_DR_Bx | AMD_FAM15_ALL))
	...
	if (mctGet_NVbits(NV_ALL_MEM_CLKS) == 0) {
		...
		if (!(pDCTstat->status & (1 << SB_DIAG_CLKS))) {
			...
			while (dword < MAX_CS_SUPPORTED) {
				if (pDCTstat->cs_present & (1 << dword)) {
				...
				struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	pDCTstat->CSPresent_DCT[dct] = pDCTstat->cs_present;
	/* dump_pci_device(PCI_DEV(0, 0x18+pDCTstat->node_id, 2)); */
				...
				struct DCTStatStruc *pDCTstat)
{
	...
	if (pDCTstat->dimm_valid != 0) {	/* If any DIMMs are present...*/
		...
		if (LRDIMMPresent != 0) {
			if ((LRDIMMPresent ^ pDCTstat->dimm_valid) !=0) {
				/* module type DIMM mismatch (reg'ed, unbuffered) */
				pDCTstat->err_status |= 1 << SB_DIMM_MISMATCH_M;
				pDCTstat->err_code = SC_STOP_ERR;
			} else{
				...
				struct DCTStatStruc *pDCTstat)
{
	...
	if (byte != bytex) {
		...
		if (byte)
			pDCTstat->err_status |= (1 << SB_DIMM_MISMATCH_O); /* Set temp. to avoid setting of ganged mode */

		if ((!(pDCTstat->err_status & (1 << SB_DIMM_MISMATCH_O))) && (pDCTstat->logical_cpuid & AMD_FAM10_ALL)) {
		...
		if (byte)	/* NV_Unganged */
			pDCTstat->err_status &= ~(1 << SB_DIMM_MISMATCH_O); /* Clear so that there is no DIMM mismatch error */
	}
					...
					struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (pDCTstat->logical_cpuid & AMD_DR_GT_Bx) {
			...
			struct DCTStatStruc *pDCTstat)
{
	...
	pDCTstat->TrwtWB = pDCTstat->trwt_to;
}
				     ...
				     struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (pDCTstat->logical_cpuid & (AMD_DR_DAC2_OR_C3 | AMD_RB_C3 | AMD_FAM15_ALL)) {
		if (is_fam15h()) {
	...
	if (pDCTstat->logical_cpuid & (AMD_DR_DAC2_OR_C3 | AMD_FAM15_ALL)) {
		if (pDCTstat->dimm_ecc_present == 0) {
				...
				struct DCTStatStruc *pDCTstatA)
{
	...
	if (pDCTstat->logical_cpuid & AMD_DR_ALL) {
					...
					struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (!is_fam15h()) {
		...
		*/
		...
		if (pDCTstat->logical_cpuid & (AMD_DR_Cx | AMD_DR_Dx))
			val |= (1 << EARLY_ARB_EN);
					...
					struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (is_fam15h()) {
	...
	} else if (pDCTstat->logical_cpuid & AMD_DR_Dx) {
		if (pDCTstat->speed == 3)
			dword = 0x00000800;
		else
			dword = 0x00000000;
		for (i = 0; i < 2; i++) {
			...
			/* Obtain number of DIMMs on channel */
			u8 dimm_count = pDCTstat->ma_dimms[i];
			u8 rank_count_dimm0;
			u8 rank_count_dimm1;
			u32 odt_pattern_0;
			u32 odt_pattern_1;
				...
				struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (pDCTstat->logical_cpuid & (AMD_DR_DAC2_OR_C3)) {
				...
				struct DCTStatStruc *pDCTstat, u8 dct)
{
	...
	if (mctGet_NVbits(NV_CKE_CTL)) {
		if (pDCTstat->cs_present == 3) {
			u16 word;
			word = pDCTstat->dimm_spd_checksum_err;
			if (dct == 0)
				word &= 0b01010100;
			else
				word &= 0b10101000;
