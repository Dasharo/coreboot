...
/*
 * A helper structure which bundles physical block address with its data.
 *
 * Usage:
					      ...
					      uint64_t *total_data_size)
{
	...
	while (!is_final_block(&block)) {
		...
		while (size_left != 0) {
			/* is_good_block() holds here whether its the first iteration or not. */
				   ...
				   int max_blocks)
{
	...
	for (int i = 0; block_count < max_blocks; ++i) {
		...
		enum cb_err ret = efi_fv_get_option(rdev, &global_variables_guid, var_name,
						    &block.self, &size);
		if (ret != CB_SUCCESS) {
			/* No more variables. */
			break;
...
static struct memory_range pick_buffer(uint64_t total_data_size)
{
	...
	memranges_each_entry(r, &memory_map) {
		...
		/* Don't allocate the buffer too low. */
		if (base < 1 * MiB) {
			if (size <= 1 * MiB)
				continue;
		...
		if (size < total_data_size)
			continue;
		if (base + total_data_size > 4ULL * GiB)
			continue;

		buffer.base = base;
		buffer.len = total_data_size;

		/* Keep looking for the highest block to not create troubles for payloads. */
	}
...
void efi_parse_capsules(void)
{
	...
	if (coalesce_buffer.base == 0) {
		printk(BIOS_ERR,
		       "capsules: failed to find a buffer for coalesced UEFI capsules.\n");
	} else {
		printk(BIOS_DEBUG, "capsules: coalesting capsules data @ %#010x.\n",
...
void lb_uefi_capsules(struct lb_header *header)
{
	...
	for (i = 0; i < uefi_capsule_count; ++i) {
		...
		printk(BIOS_INFO, "capsules: publishing a capsule @ %#010llx.\n",
		       capsule->range_start);
