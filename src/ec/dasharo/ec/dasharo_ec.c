...
uint8_t dasharo_ec_smfi_cmd(uint8_t cmd, uint8_t len, uint8_t *data)
{
	...
	// Write data first
	for (i = 0; i < len; ++i)
		dasharo_ec_write(REG_CMD + 2 + i, data[i]);
...
static uint8_t ec_spi_reset(void)
{
	...
	return dasharo_ec_smfi_cmd(CMD_SPI, 1, &reset_cmd);
}
...
static int ec_spi_bus_write(uint8_t *data, uint8_t len)
{
	...
	for (addr = 0; addr + SPI_ACCESS_SIZE < len; addr += SPI_ACCESS_SIZE) {
		...
		for (i = 0; i < write_cmd[1]; ++i)
			dasharo_ec_write(REG_DATA + 2 + i, data[addr + i]);

		if ((rv = dasharo_ec_smfi_cmd(CMD_SPI, ARRAY_SIZE(write_cmd), write_cmd)))
			return -rv;
...
static int ec_spi_erase_chip(void)
{
	...
	for (addr = 0; addr < CONFIG_EC_DASHARO_EC_FLASH_SIZE; addr += SPI_SECTOR_SIZE)
...
static int ec_spi_image_verify(uint8_t *image, ssize_t image_sz)
{
	...
	for (addr = 0;
	     addr < CONFIG_EC_DASHARO_EC_FLASH_SIZE && addr + SPI_SECTOR_SIZE < image_sz;
...
static void dasharo_ec_fw_sync(void *unused)
{
	...
	if (!(image_sz = cbfs_load("ec.rom", image, CONFIG_EC_DASHARO_EC_FLASH_SIZE))) {
	...
	if (!image_sz || image_sz % 2 || image_sz > CONFIG_EC_DASHARO_EC_FLASH_SIZE) {
	...
	if (dasharo_ec_smfi_cmd(CMD_SPI, 1, &smfi_cmd)) {
	...
	smfi_cmd = 0;
	if (dasharo_ec_smfi_cmd(CMD_RESET, 1, &smfi_cmd)) {
...
BOOT_STATE_INIT_ENTRY(BS_PRE_DEVICE, BS_ON_ENTRY, dasharo_ec_fw_sync, NULL);
