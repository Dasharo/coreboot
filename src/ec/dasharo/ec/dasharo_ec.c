...
uint8_t dasharo_ec_smfi_cmd(uint8_t cmd, uint8_t len, uint8_t *data)
{
	...
	if (len > DASHARO_EC_SIZE - REG_DATA) {
		printk(BIOS_ERR, "dasharo_ec: Invalid command length\n");
		return -1;
	}

	// Wait for previous command completion, for up to 10 milliseconds, with a
	// test period of 1 microsecond
	wait_us(SPI_TIMEOUT_10MS, dasharo_ec_read(REG_CMD) == CMD_FINISHED);
...
uint8_t dasharo_ec_read_version(uint8_t *data)
{
	...
	// Write command register, which starts command
	dasharo_ec_write(REG_CMD, CMD_VERSION);

	// Wait for previous command completion, for up to 10 milliseconds, with a
	// test period of 1 microsecond
	wait_us(SPI_TIMEOUT_10MS, dasharo_ec_read(REG_CMD) == CMD_FINISHED);

	result = dasharo_ec_read(REG_RESULT);

	if (result != 0)
		return result;

	// Read data bytes, index should be valid due to length test above
	for (i = 0; i < (DASHARO_EC_SIZE - REG_DATA); i++) {
		data[i] = dasharo_ec_read(REG_DATA + i);
		if (data[i] == '\0')
			break;
	}
...
uint8_t dasharo_ec_read_board(uint8_t *data)
{
	...
	// Write command register, which starts command
	dasharo_ec_write(REG_CMD, CMD_BOARD);
...
static int ec_spi_bus_read(uint8_t *dest, uint32_t len)
{
	...
	for (addr = 0; addr + SPI_ACCESS_SIZE < len; addr += SPI_ACCESS_SIZE) {
		...
		if ((rv = dasharo_ec_smfi_cmd(CMD_SPI, ARRAY_SIZE(read_cmd), read_cmd))) {
			printk(BIOS_ERR, "dasharo_ec: Failed to send read SPI bus command\n");
			return -rv;
		}

		if (dasharo_ec_read(REG_DATA + 1) != read_cmd[1]) {
			printk(BIOS_ERR, "dasharo_ec: SPI bus read insufficient bytes\n");
			return -1;
		}

		for (i = 0; i < read_cmd[1]; i++)
			dest[addr + i] = dasharo_ec_read(REG_DATA + 2 + i);
	}
	...
	if ((rv = dasharo_ec_smfi_cmd(CMD_SPI, ARRAY_SIZE(read_cmd), read_cmd))) {
		printk(BIOS_ERR, "dasharo_ec: Failed to send read SPI bus command (remainder)\n");
		return -rv;
	}

	if (dasharo_ec_read(REG_DATA + 1) != read_cmd[1]) {
		printk(BIOS_ERR, "dasharo_ec: SPI bus read remainder insufficient bytes\n");
		return -1;
	}
...
static int ec_spi_bus_write_with_flags(uint8_t *data, uint8_t len, uint8_t flags)
{
	...
	for (addr = 0; addr + SPI_ACCESS_SIZE < len; addr += SPI_ACCESS_SIZE) {
		...
		for (i = 0; i < write_cmd[1]; ++i)
			dasharo_ec_write(REG_DATA + ARRAY_SIZE(write_cmd) + i, data[addr + i]);

		rv = dasharo_ec_smfi_cmd(CMD_SPI, ARRAY_SIZE(write_cmd), write_cmd);
		if (rv) {
			printk(BIOS_ERR, "dasharo_ec: Failed to send write SPI bus command\n");
			return -rv;
		}

		if (dasharo_ec_read(REG_DATA + 1) != write_cmd[1]) {
			printk(BIOS_ERR, "dasharo_ec: SPI bus write insufficient bytes\n");
			return -1;
		}
	}
	...
	rv = dasharo_ec_smfi_cmd(CMD_SPI, ARRAY_SIZE(write_cmd), write_cmd);
	if (rv) {
		printk(BIOS_ERR, "dasharo_ec: Failed to send write SPI bus command (remainder)\n");
		return -rv;
	}

	if (dasharo_ec_read(REG_DATA + 1) != write_cmd[1]) {
		printk(BIOS_ERR, "dasharo_ec: SPI bus write remainder insufficient bytes\n");
		return -1;
	}
...
static int ec_spi_image_write(uint8_t *image, size_t size)
{
	uint8_t *sector;
	uint32_t addr;
	uint32_t erase_addr = CONFIG_EC_DASHARO_EC_FLASH_SIZE;
	size_t length;
	int rv;
	...
	while ((addr < CONFIG_EC_DASHARO_EC_FLASH_SIZE) &&
	...
	if (size % SPI_SECTOR_SIZE == 0 && addr < CONFIG_EC_DASHARO_EC_FLASH_SIZE)
	...
	while (erase_addr < CONFIG_EC_DASHARO_EC_FLASH_SIZE) {
...
static void dasharo_ec_fw_sync(void *unused)
{
	size_t image_sz;
	char *image;
	...
	/* Sanity checks */
	if (image_sz % 2 || image_sz > CONFIG_EC_DASHARO_EC_FLASH_SIZE) {
	...
	dasharo_ec_read_board((uint8_t *)cur_board_str);
