...
static enum cb_err get_ibbs_flags(bool *auth_measure, bool *srtm_ac)
{
	size_t bpm_len;
	...
	*srtm_ac = (ibbs->flags & BPM_IBBS_FLAG_SRTM_AC) != 0;
...
void intel_cbnt_inject_ibg_measurements(void)
{
	...
	bool auth_measure;
	bool srtm_ac;
	if (get_ibbs_flags(&auth_measure, &srtm_ac) != CB_SUCCESS) {
	...
	(void)obuf_write_le64(&data_ob,
			      srtm_ac ? biosacm_sts.raw & SRTM_AC_MASK : biosacm_sts.raw);
