...
void bootblock_mainboard_early_init(void)
{
	...
	ite_reg_write(GPIO_DEV, 0x26, CONFIG(BOARD_PROTECTLI_AP2110) ? 0xc4 : 0xc0);
	...
	if (CONFIG(BOARD_PROTECTLI_AP2110)) {
		...
		ite_gpio_setup(GPIO_DEV, 12, ITE_GPIO_INPUT, ITE_GPIO_SIMPLE_IO_MODE,
			       ITE_GPIO_CONTROL_DEFAULT);
	}
	...
	/*
	 * Initialize COM0 I/O base to avoid duplicated I/O base between the
	 * serial ports during ramstage device initialization resulting in a
	 * slowed boot with debug build.
	 */
	if (CONFIG(BOARD_PROTECTLI_VP2430))
		ite_enable_serial(PNP_DEV(0x2e, IT8659E_SP1), 0x2f8);
}
