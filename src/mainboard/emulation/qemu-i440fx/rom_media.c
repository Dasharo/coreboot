 ...
 *   parts, like OVMF_CODE.fd and OVMF_VARS.fd. Combined size of system firmware
 *   must not exceed 8 MiB (FW image(s) mounted with '-drive if=pflash').
 *
...
void boot_device_init(void)
{
	...
	/* Detect what type of flash we're dealing with. */
	ptr = (volatile char *)((uintptr_t)boot_dev.base + 0x10000);
	...
	*ptr = CLEAR_STATUS_CMD;
	readback = *ptr;
	if ((original != CLEAR_STATUS_CMD) && (readback == CLEAR_STATUS_CMD))
	{
		printk(BIOS_DEBUG, "QEMU flash behaves as RAM\n");
		/* Restore original content. */
		*ptr = original;
	} else {
		/* In case original value was CLEAR_STATUS_CMD, RAM is checked again. */
		*ptr = READ_STATUS_CMD;
		readback = *ptr;
		if (readback == original) {
		...
		} else if (readback == READ_STATUS_CMD) {
			printk(BIOS_DEBUG, "QEMU flash behaves as RAM\n");
			/* Restore original content. */
			*ptr = original;
			...
			*ptr = READ_ARRAY_CMD;
			...
			printk(BIOS_ERR, "Unexpected QEMU flash behavior\n");
			/*
