...
static void add_cbmem_pointers(struct lb_header *header)
{
	...
	const struct section_id {
		...
		{CBMEM_ID_TCPA_TCG_LOG, LB_TAG_TCPA_SPEC_LOG},
...
static unsigned long lb_table_fini(struct lb_header *head)
{
	...
	head->table_checksum = htole32(compute_ip_checksum(first_rec,
				       le32toh(head->table_bytes)));
	head->header_checksum = 0;
	head->header_checksum = htole32(compute_ip_checksum(head, sizeof(*head)));
	printk(BIOS_DEBUG,
