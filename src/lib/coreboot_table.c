...
static void lb_add_serial(struct lb_header *header)
{
	...
	new_serial.tag         = htole32(new_serial.tag);
	new_serial.size        = htole32(new_serial.size);
	new_serial.type        = htole32(new_serial.type);
	new_serial.baseaddr    = htole32(new_serial.baseaddr);
	new_serial.baud        = htole32(new_serial.baud);
	new_serial.regwidth    = htole32(new_serial.regwidth);
	new_serial.input_hertz = htole32(new_serial.input_hertz);

	struct lb_serial *serial = (struct lb_serial *)lb_new_record(header);
	memcpy(serial, &new_serial, sizeof(*serial));
	assert(serial->type == htole32(LB_SERIAL_TYPE_IO_MAPPED)
	       || serial->type == htole32(LB_SERIAL_TYPE_MEMORY_MAPPED))
	if (serial->type == htole32(LB_SERIAL_TYPE_IO_MAPPED))
