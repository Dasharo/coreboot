...
static int smm_place_entry_code(const unsigned int num_cpus)
{
	unsigned int i;
	unsigned int size;
...
static int smm_stub_place_staggered_entry_points(const struct smm_loader_params *params)
{
	...
	return smm_place_entry_code(params->num_concurrent_save_states);
}
				...
				struct smm_loader_params *params)
{
	...
	if (fx_save_area_size) {
		...
		if (fx_save_area_base < cpus[0].code_end) {
			printk(BIOS_ERR, "fxsave wont fit in smram\n");
			return -1;
		}
	}
