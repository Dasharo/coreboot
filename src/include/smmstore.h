 ...
 *
 * To prevent malicious ring0 code to access arbitrary memory locations, a
 * communication buffer in CBMEM/HOB is used for all transfers.
 * The communication buffer has to be at least 64KiB in size and must be installed
 * before the SMMSTORE's read or write operation are called.
 * Usually coreboot or the payload will install a communication buffer.
 *
 * The Version 2 protocol separates the SMMSTORE into 64KiB blocks, where each block
