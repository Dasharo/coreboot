...
usage() {
  ...
  echo -e "\tgigabyte               - build Dasharo for Gigabyte MZ33 AR1"
...
function build_gigabyte {
  DEFCONFIG="configs/config.gigabyte_mz33-ar1"

  build_prep

  echo "Building Dasharo for Gigabyte MX33 AR"

  build_start

  cp build/coreboot.rom gigabyte.rom

  if [ $? -eq 0 ]; then
    echo "Result binary placed in $PWD/gigabyte.rom"
    sha256sum gigabyte.rom > gigabyte.rom.sha256
  else
    echo "Build failed!"
    exit 1
  fi
